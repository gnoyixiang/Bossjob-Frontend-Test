...
const withPagination = (WrappedComponent, options = {}) => class WithPagination extends Component {
    ...
    setPagination = (pagination, onPaginationChange) => {
        ...
        if (actualPage < 1) {
            actualPage = 1;
        } else if (actualPage > Math.ceil(total / size) || actualPage > Math.ceil(this.state.total / this.state.size)) {
            actualPage = Math.ceil(total / size);
        }
        console.log('setPagination', actualPage, pagination, this.state);
        this.setState({
            page: actualPage,
