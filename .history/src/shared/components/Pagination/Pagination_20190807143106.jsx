            ...
            {displayedPages.map((pageNum, index) => (
                <React.Fragment>
                    {pageNum > displayedPages[index - 1] + 1 ? (
                        <span key={`${displayedPages[index - 1]}_to_${pageNum}`} className={classnames("Pagination-ellipsis")}>...</span>
                    ) : null}
                    <span key={pageNum+index} className={classnames("Pagination-page", page === pageNum ? "Pagination-active" : "")}>{pageNum}</span>
                </React.Fragment>
            ))}
