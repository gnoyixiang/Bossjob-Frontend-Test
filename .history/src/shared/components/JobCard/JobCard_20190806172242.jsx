...
const JobCard = props => {
    ...
    const locale = window.navigator.language.split('-')[0] + '-' + (countryList.getCode(job_country) || window.navigator.language.split('-')[1]);
    const countryName = currencyCodes.countries().find(name => name.toLowerCase().indexOf(job_country.toLowerCase()) >= 0);
        ...
        <div className={classnames(className, 'JobCard')} {...restProps}>
            <div className="JobHeader">
                ...
                <div className="Salary">
                    {Intl.NumberFormat(...intlNumberFormatParams).format(salary_range_from)} - {Intl.NumberFormat(...intlNumberFormatParams).format(salary_range_to)}
                </div>
            </div>
