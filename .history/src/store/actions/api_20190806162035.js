...
export const searchJobs = (options) => {
    return (dispatch) => {
        ...
        return api.searchJobs(options)
            .then(response => {
                console.log('searchJobs response', response.config, response.request, response.response);
                if (response.request.status !== 200) {
                    console.log('searchJobs response error', response.config, response.request, response.response);
